 ...
 *
 * For ad-hoc tasks (requireEvidence=true), completion signals alone
 * are not enough — we also need observable evidence (files changed,
...
export function isWPCompleted(report: RunReport | null, requireEvidence = false): boolean {
  ...
  // Check summary for completion signals
  if (report.summary) {
    const lower = report.summary.toLowerCase();
    if (lower.includes('completed successfully') || lower.includes('exit code: 0')) {
  ...
  // For ad-hoc tasks: require concrete evidence beyond just saying "completed"
  if (requireEvidence) {
    ...
    return hasFilesChanged || hasFixApplied || hasVerification || hasImplementation;
  }
